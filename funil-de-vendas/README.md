...
```bash
npm install                      # instala Jest, Playwright, ESLint, Prettier e Husky
...
suíte de testes) **antes de cada commit, em qualquer branch** — código com erro
de lint ou fora do padrão não entra no histórico. Os arquivos `.gitignore`,
`.eslintignore` e `.prettierignore` mantêm `node_modules/` e artefatos de teste
fora do repositório (o HTML e os `.md` ficam fora do Prettier, por serem
formatados à mão).
...
```bash
git init                                   # 1. inicia o repositório (o .gitignore já protege o que não sobe)
...
gh repo create funil-de-vendas --public --source=. --push   # 4. cria o repo remoto e sobe (ou crie pelo site e git push)
```
